...
Values for the *Below* and *Above* observations as read on the sextant, in decimal arc minutes (*MM.m*), can be entered in the input form. **Enter the positive value directly read off the minute dial in the fields. Do not enter the degree part of the readings, which should be zero (if the degree value is not zero, your sextant is too far off to calculate a correction using this method, it should be calibrated first). Do NOT correct the off scale *Below* reading by subtracting it from 60 or by entering a negative number, the program will do that for you. Use the positive value directly read off of the minute dial (no degrees).**
