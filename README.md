...
This repository contains a set of programs written in Prime Programming Language (PPL) for the [HP Prime calculator V2](https://www.hpcc.org/calculators/hpprime.html) to turn it into a reliable, low-cost celestial navigation computer that can live in a chart table and requires no updates until the year 2200 (NOTE: there are issues with Prime G2 support currently that render it unusable on this specific flavor of Prime and which are being worked on). It can also be used on any PC or Mac running the [*HP Prime Virtual Calculator*](https://updates.moravia-consulting.com/) program. You may freely use and modify this program for non-commercial purposes under this [license](https://github.com/placidanomaly/HPPrimeSightReduction/blob/main/LICENSE)
