...
The first thing to do is to initialize lists and variables, set default values, and set the time and timezone of the calculator. This must also be done every time after opening a program in editing mode even if no changes were made to it. If you ever get an **Invalid Input** error (as you would if you try to run the program after editing it without resetting it first), you need to re-initialize the program. Proceed as follows:
