...
Whenever a new sight is entered, a value for the index correction of the sextant must be specified. Because the index correction of a sextant usually remains the same for all observations, a default value for the index correction can be set. This can be done by directly specifying the value of the index correction, for example the value read on the sextant after aligning the reflection of a faraway object or the horizon with the direct image of the same, or by calculating it based on a measurement of the diameter of the sun, which seems to give more accurate values. At the end of the procedure, the value of the index correction for all new sights will be set to the new default index correction value (i.e. the index correction of existing sights will not be changed).

To set the default index correction, from the main menu, select **0. Setup** then **1. Sextant Index Correction**. Then press the soft key corresponding to the desired method: either the *Direct* method to enter the default index correction directly, or the *Solar* method, which will launch the solar diameter measurement process.

![Index Correction options](https://github.com/placidanomaly/HPPrimeSightReduction/blob/main/img/index_correction.png)

### Direct Entry

When selecting the Direct soft key, the following input screen appears. Enter the default index correction in the field in decimal minutes (*MM.m*), with a negative value for an on-scale correction and a positive value for an off-scale correction. The degree value must not be entered as it is assumed to be zero. If your index correction is larger than 1 degree, your sextant should be calibrated such that the index correction will be smaller than 1 degree. 
...
Calculating the index correction from a measurement of the diameter of the Sun is accomplished by following the procedure described in [this document](https://www.starpath.com/online/celestial/solar_ic.pdf). The procedure assumes that the index correction is small enough such that the top measurement is on-scale and the bottom measurement is off-scale. If that is not the case, your sextant must be calibrated before using this procedure. The program briefly describes the measurements that need to be taken:

![Solar IC intro](https://github.com/placidanomaly/HPPrimeSightReduction/blob/main/img/solaric_intro.png)

Values for the *Below* and *Above* observations, in decimal arc minutes (*MM.m*), can be entered in the input form. **Enter the positive value read off the minute dial in the fields. Do not enter the degree part of the readings, which should be zero (if the degree value is not zero, your sextant is too far off to calculate a correction using this method, it should be calibrated first). Do NOT correct the off scale *Below* reading by subtracting it from 60 or by entering a negative number, the program will do that for you. Just use the positive value directly read off of the minute dial of the sextant.**

The order in which the *Below* and *Above* values are measured should match the direction in which you align celestial bodies with the horizon during a sight. You can easily see why by performing the procedure in one direction first (e.g. measuring the *Below* and *Above* values in that order while raising the reflected view), and then in the opposite direction (e.g. measuring the *Above* and *Below* values in that order while lowering the reflected view). Based on the quality of your sextant and the amount of backlash in the mechanism, you will likely get two very different values of index correction. So if you typically bring the reflected celestial bodies DOWN to the horizon when making a sight, do this procedure by moving the sextant down, i.e. start with the reflected sun above the actual sun and bring it down to read the *Above* value first, and then down again to read the *Below* value. If you do it the other way around, i.e. you bring the reflected celestial bodies UP to the horizon, do the procedure from the bottom to the top instead.
