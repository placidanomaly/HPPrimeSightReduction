...
The first thing to do is to initialize lists and variables, set default values, and set the time and timezone of the calculator. <u>A reset must also be done every time the Sight Reduction program is opened in editing mode even if no changes are made to it.</u> If you ever get an **Invalid Input** error (as you would if you try to run the program after editing it without running the reset function afterward), you need to re-run the reset procedure. Finally, you'll want to perform the reset whenever starting a new fix to remove the lines of position of the previous fix. Proceed as follows:
